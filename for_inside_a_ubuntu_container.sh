...
# Set the hostname to localhost (or IP if needed)
...
# Install necessary packages for Apache, PHP, and SQLite
echo "=== Installing Apache, PHP, SQLite ==="
apt -y install apache2 php php-sqlite3 php-pdo php-mbstring sqlite3
...
# Ubuntu SELinux handling: Usually not needed for Ubuntu since SELinux is not the default on Ubuntu, but if you're running it with SELinux enabled, this might be needed
# To make sure the web server can write to the db file (Ubuntu typically doesn't enforce SELinux, but it's worth checking in case it's enabled)
echo "=== Applying SELinux context for writable DB (if applicable) ==="
...
# Display public IP of the container (use the container's IP if it's running as part of a Docker network)
